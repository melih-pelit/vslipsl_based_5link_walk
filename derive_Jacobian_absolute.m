...
% 2019.12.17 modify to use absolute coordinates!
clear all
clc
...
x_G(2) = x1 + l1*cos(th1) + (l1/2)*cos(th2);
x_G(3) = x1 + l1*cos(th1) + l2*cos(th2) + (l3/2)*cos(th3);
x_G(4) = x1 + l1*cos(th1) + l2*cos(th2) + l3*cos(th3) + (l4/2)*cos(th4);
x_G(5) = x1 + l1*cos(th1) + l2*cos(th2) + (l5/2)*cos(th5); % body

% z_g CoM's of respective links
z_G = sym(zeros(5));
z_G(1) = (l1/2)*sin(th1); 
z_G(2) = l1*sin(th1) + (l2/2)*sin(th2);
z_G(3) = l1*sin(th1) + l2*sin(th2) + (l3/2)*sin(th3);
z_G(4) = l1*sin(th1) + l2*sin(th2) + l3*sin(th3) + (l4/2)*sin(th4);
z_G(5) = l1*sin(th1) + l2*sin(th2) + (l5/2)*sin(th5);
...
x_F = x1 + l1*cos(th1) + l2*cos(th2) + l3*cos(th3) + l4*cos(th4);
z_F = l1*sin(th1) + l2*sin(th2) + l3*sin(th3) + l4*sin(th4);
...
% coordinates here are absolute coordinates
syms foot foot_minus % respectively stance foot and prev stance foot loc.

q_st = [th1; th2];
dq_st = [dth1; dth2];
x_hip_st = foot + l1*cos(th1) + l2*cos(th2);
z_hip_st = l1*sin(th1) + l2*sin(th2);
...
q_sw = [th3; th4];
dq_sw = [dth3; dth4];
% x_hip_sw = l4 * cos(th4) + l3*cos(th3);
% z_hip_sw = l4 * sin(th4) + l3*sin(th3);
x_hip_sw = foot_minus - l4 * cos(th4) - l3*cos(th3);
z_hip_sw = - l4 * sin(th4) - l3*sin(th3);
...
for i = 1:1:Gamma_st_Size(1)
    for j = 1:1:Gamma_st_Size(2)
        dGamma_st(i,j) = simplify(jacobian(Gamma_st(i, j), q) * dq);
    end
end
...
for i = 1:1:Gamma_sw_Size(1)
    for j = 1:1:Gamma_sw_Size(2)
        dGamma_sw(i,j) = simplify(jacobian(Gamma_sw(i, j), q) * dq);
    end
end

%% Calculating the acceleration of CoM of 5 Link robot
